...
function(yi, ind.lst, X, twosex, sex, ni, ni0, xs, start, iphi=1, theta=1, maxiter=100, eps=1E-3, tech.ctrl){
  ...
  repeat{
    ...
    tag = tryCatch({    
      iphi_i = iphi
      theta_i = theta
      log.lik0 = ll.jRCI.A.cons(par0, yi=yi, ind.lst=ind.lst, X=X, twosex=twosex, sex=sex,  
            ni=ni, ni0=ni0, xs=xs, iphi=iphi_i, theta=theta_i)
      for(i in 1:maxiter){
        out = optim(par0, ll.jRCI.A.cons, yi=yi, ind.lst=ind.lst, X=X, twosex=twosex, sex=sex, 
            ni=ni, ni0=ni0, xs=xs, iphi=iphi_i, theta=theta_i)
        par0 = out$par
        log.lik1 = ll.jRCI.A.cons(par0, yi=yi, ind.lst=ind.lst, X=X, twosex=twosex, sex=sex, 
            ni=ni, ni0=ni0, xs=xs, iphi=iphi_i, theta=theta_i)    
        older = c(par0, log.lik1, iphi_i, theta_i)    
        if(twosex){
          pari = c(par0[1:2], par0)
        }else{
          pari = c(par0[1], par0)
        }
        iphi_i = optimize(f=ll.tRCI.iphi.A, c(tech.ctrl$iphi_l, tech.ctrl$iphi_u), 
              yi=yi, ind.lst=ind.lst, X=X, twosex=twosex, betas=pari)$minimum
