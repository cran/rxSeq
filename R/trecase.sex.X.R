...
function(yi, ind.lst, X, ni, ni0, xs, l.tau.r, l.tau.a, 
  ...
  repeat{
    ...
    tag = tryCatch({
      iphi_i = iphi
      theta_i = theta
      log.lik0 = ll.jRCI.sex.X(par0, yi=yi, ind.lst=ind.lst, X=X, 
            ni=ni, ni0=ni0, xs=xs, iphi=iphi_i, theta=theta_i, l.tau.r=l.tau.r, l.tau.a=l.tau.a)
      for(i in 1:maxiter){
        out = optim(par0, ll.jRCI.sex.X, yi=yi, ind.lst=ind.lst, X=X, 
            ni=ni, ni0=ni0, xs=xs, iphi=iphi_i, theta=theta_i, l.tau.r=l.tau.r, l.tau.a=l.tau.a)
        log.lik1 = ll.jRCI.sex.X(par0, yi=yi, ind.lst=ind.lst, X=X, 
            ni=ni, ni0=ni0, xs=xs, iphi=iphi_i, theta=theta_i, l.tau.r=l.tau.r, l.tau.a=l.tau.a)    
        older = c(par0[1], par0[1:5], 0, par0[6:7], log.lik1, iphi_i, theta_i)   
        par0 = out$par
        iphi_i = optimize(f=ll.tRCI.iphi.X, c(tech.ctrl$iphi_l, tech.ctrl$iphi_u), 
              yi=yi, ind.lst=ind.lst, X=X, twosex=TRUE, betas=older[c(1:6, 8:9)], l.tau.r=l.tau.r, l.tau.a=l.tau.a)$minimum
        ...
        log.lik1 = ll.jRCI.sex.X(par0, yi=yi, ind.lst=ind.lst, X=X, 
            ni=ni, ni0=ni0, xs=xs, iphi=iphi_i, theta=theta_i, l.tau.r=l.tau.r, l.tau.a=l.tau.a)    
        newer = c(par0[1], par0[1:5], 0, par0[6:7], log.lik1, iphi_i, theta_i)       
